...
[dispatchWorker, inappWorker, telegramWorker, emailWorker, ccAttachmentsWorker].forEach((w) => {
	w.on("failed", (job, err) => {
...
async function shutdown() {
	await Promise.allSettled([dispatchWorker.close(), inappWorker.close(), telegramWorker.close(), emailWorker.close(), ccAttachmentsWorker.close()]);
	process.exit(0);
}
