...
/**
 * Редагування вихідного повідомлення менеджера.
 * Право: тільки текстове out-повідомлення цього менеджера, не видалене.
 * Повертає { ok, source_id } — source_id потрібен, щоб оновити стару веб-чат схему.
 */
async function editOutgoingMessage(idMessage, idManager, text) {
	const [rows] = await connection_pool.execute(`SELECT id, direction, type, id_manager, source_id, id_conversation, date_deleted FROM ${T_MESSAGES} WHERE id = ? LIMIT 1`, [idMessage]);
	if (!rows.length) return { ok: false, error: "not_found" };
	const m = rows[0];
	if (m.direction !== "out" || m.type !== "text" || m.date_deleted) return { ok: false, error: "not_editable" };
	if (Number(m.id_manager) !== Number(idManager)) return { ok: false, error: "not_owner" };

	await connection_pool.execute(`UPDATE ${T_MESSAGES} SET text = ?, date_edited = NOW(3) WHERE id = ?`, [text, idMessage]);
	return { ok: true, source_id: m.source_id, id_conversation: m.id_conversation };
}
