...
async function webchatRefByConversation(idConversation) {
	const [rows] = await connection_pool.query(
		`SELECT ct.external_id, ct.id_channel, ch.type AS channel_type
           FROM ${P}contact_center_conversations AS c
           INNER JOIN ${P}contact_center_channels AS ch ON ch.id = c.id_channel
           INNER JOIN ${P}contact_center_contacts AS ct ON ct.id = c.id_contact
          WHERE c.id = ? LIMIT 1`,
	...
	// roomId = "<siteId>_v_<visitorHex>". siteId САМ містить "_",
	// тому ділимо по роздільнику "_v_", а не по першому "_".
	const idx = roomId.indexOf("_v_");
	const site_id = idx > -1 ? roomId.slice(0, idx) : null;

	return { site_id: site_id, room_id: roomId };
}
...
// Менеджер друкує (CRM) → у веб-чат клієнту
async function operatorTyping(idConversation) {
	const ref = await webchatRefByConversation(idConversation);
	if (!ref || !ref.site_id) return;
	const webchat = require("../../routes/contact-center/web-chat/web-chat");
	if (typeof webchat.typingFromCrm === "function") webchat.typingFromCrm(ref.site_id, ref.room_id);
}

// Менеджер відредагував повідомлення (CRM) → у стару веб-чат схему й клієнту.
// ccSourceId = "wc_<oldId>" з contact_center_messages.source_id.
async function editWebchatMessage(idConversation, ccSourceId, text) {
	const ref = await webchatRefByConversation(idConversation);
	if (!ref || !ref.site_id) return { ok: false };
	const m = String(ccSourceId || "").match(/^wc_(\d+)$/);
	if (!m) return { ok: false, error: "not_webchat_msg" };
	const oldMsgId = parseInt(m[1], 10);
	const webchat = require("../../routes/contact-center/web-chat/web-chat");
	if (typeof webchat.editMessageFromCrm !== "function") return { ok: false };
	return webchat.editMessageFromCrm(ref.site_id, ref.room_id, oldMsgId, text);
}

// Список форм каналу (для CRM-діалогу)
async function formsByConversation(idConversation) {
	const ref = await webchatRefByConversation(idConversation);
	if (!ref || !ref.site_id) return [];
	const webchat = require("../../routes/contact-center/web-chat/web-chat");
	if (typeof webchat.getFormsForCrm !== "function") return [];
	return webchat.getFormsForCrm(ref.site_id);
}

// Надіслати форму клієнту (з CRM-діалогу)
async function sendFormByConversation(idConversation, formId, managerId) {
	const ref = await webchatRefByConversation(idConversation);
	if (!ref || !ref.site_id) return { ok: false, error: "Не веб-чат діалог" };
	const webchat = require("../../routes/contact-center/web-chat/web-chat");
	if (typeof webchat.sendFormFromCrm !== "function") return { ok: false, error: "Недоступно" };
	return webchat.sendFormFromCrm(ref.site_id, ref.room_id, formId, managerId);
}

module.exports = { mirror, channelBySite, markRead, typing, presence, product, productsForConversation, isConversationOnline, onlineConversationIds, visitorInfoForConversation, visitorPage, operatorTyping, editWebchatMessage, formsByConversation, sendFormByConversation };
