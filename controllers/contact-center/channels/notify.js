...
// Формат токена Telegram — жорстко. Будь-що інше НЕ пускаємо у fetch.
const TG_TOKEN_RE = /^\d{6,12}:[A-Za-z0-9_-]{30,}$/;
// Дозволені методи Bot API, які ми реально викликаємо. Whitelist замість вільного рядка.
const TG_METHODS = new Set(["getMe", "sendMessage", "sendPhoto", "sendVideo", "sendAudio", "sendDocument"]);

async function tgCall(token, method, params) {
	// 1. Валідація токена: захист від інʼєкції в структуру URL (SSRF-патерн)
	if (typeof token !== "string" || !TG_TOKEN_RE.test(token)) {
		return { ok: false, description: "invalid_token_format" };
	}
	// 2. Метод — лише з нашого whitelist, а не довільний рядок
	if (!TG_METHODS.has(method)) {
		return { ok: false, description: "method_not_allowed" };
	}

	// 3. Збираємо URL безпечно. Хост фіксований і незмінний.
	//    Токен уже провалідований регексом (без /, @, ?, #, пробілів) — у path він безпечний.
	...
	try {
		const res = await fetch(url, {
			...
			redirect: "error", // 4. Забороняємо редиректи — щоб відповідь не могли увести на інший хост
		});
