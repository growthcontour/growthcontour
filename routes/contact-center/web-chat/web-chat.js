...
function validateConfig(cfg) {
		...
		if (!isObj(forms)) E("forms", "bad_type", "forms має бути обʼєктом-словником.");
			...
			Object.keys(forms).forEach((fid) => {
				...
				if (!Array.isArray(f.fields) || f.fields.length === 0) E(`${base}.fields`, "required", "Форма має містити хоча б одне поле.");
					...
					const KNOWN_TARGETS = ["lead", "quick", "order"];
					if (f.target !== undefined && !KNOWN_TARGETS.includes(f.target)) {
						E(`${base}.target`, "bad_enum", `target має бути одним із: ${KNOWN_TARGETS.join(", ")}.`);
					}
					// lead і order потребують контакту (щоб було кого/куди оформити)
					if (f.target === "lead" || f.target === "order") {
						...
						if (!hasContact) E(`${base}`, "lead_form_no_contact", "Форма-лід/замовлення має містити email/tel або requireAnyOf.");
					}
				}
	...
	if (isObj(bh) && bh.operatorTyping !== undefined && typeof bh.operatorTyping !== "boolean") {
		E("behavior.operatorTyping", "bad_type", "operatorTyping має бути true/false.");
	}
...
async function getSiteConfig(siteId) {
	...
	return {
		...
		// За замовчуванням увімкнено (opt-out): false лише якщо явно вимкнено
		operatorTyping: !(cfg && cfg.behavior && cfg.behavior.operatorTyping === false),
		config: cfg || {},
	};
}
...
function bindSocket(nsp) {
	...
	nsp.on("connection", (socket) => {
					...
					(async () => {
						try {
							...
							if (topic) {
								...
								// id_channel цього вебчат-каналу
								let wcIdChannel = null;
								const wcChannels = ["inapp"];
								try {
									const [chRow] = await pool.query(`SELECT id_channel FROM ${prefix}contact_center_channel_webchat WHERE site_id = ? LIMIT 1`, [d.siteId]);
									if (chRow.length) {
										wcIdChannel = chRow[0].id_channel;
										const st = await require("../../../controllers/contact-center/channels/notify").load(null, wcIdChannel);
										if (st && Number(st.push_enabled) === 1) wcChannels.push("webpush");
									}
								} catch (e) {
									console.error("wc push_enabled:", e.message);
								}

								// CRM-діалог (contact_center_conversations): і id, і url_token.
								// id → collapseKey cc_conv_<id> (щоб прочитання чистило сповіщення),
								// url_token → правильне посилання /contact-center/chat/<token>/
								let ccConvId = null;
								let ccToken = notifData.url_token;
								if (wcIdChannel != null) {
									try {
										const [ctRows] = await pool.query(
											`SELECT c.id, c.url_token
                                               FROM ${prefix}contact_center_conversations AS c
                                               INNER JOIN ${prefix}contact_center_contacts AS ct ON ct.id = c.id_contact
                                              WHERE ct.id_channel = ? AND ct.external_id = ?
                                              LIMIT 1`,
											[wcIdChannel, d.roomId]
										);
										if (ctRows.length) {
											ccConvId = ctRows[0].id;
											if (ctRows[0].url_token) ccToken = ctRows[0].url_token;
										}
									} catch (e) {
										console.error("wc cc-conv:", e.message);
									}
								}
								...
								const basePayload = {
									...
									url: `/contact-center/chat/${ccToken}/`,
									chat_id: d.roomId,
									channel: "webchat",
								...
								// Розсилка через notifyScope: розгортає отримувачів каналу (notif_recipients)
								// у групи/юзерів, які dispatch резолвить коректно. topic-шлях не працює.
								if (wcIdChannel != null) {
									const recipients = require("../../../controllers/notifications/recipients");
									recipients
										.notifyScope(
											"contact_center.channel",
											wcIdChannel,
											{
												type: "webchat.msg",
												channels: wcChannels,
												collapseKey: ccConvId ? `cc_conv_${ccConvId}` : `webchat:${d.roomId}`,
												payload: basePayload,
											},
											(o) => !!o.on_new_message || !!o.on_new_conversation
										)
										.catch((e) => console.error("notifyScope webchat:", e.message));

									// Telegram-алерт каналу (окремий бот). sendTelegram сам перевіряє tg_enabled.
									try {
										const ccNotify = require("../../../controllers/contact-center/channels/notify");
										const escTg = (s) =>
											String(s || "")
												.replace(/&/g, "&amp;")
												.replace(/</g, "&lt;")
										...
										const link = base + "/contact-center/chat/" + ccToken + "/";
										const tgText = `💬 <b>Новий чат (веб)</b>\n${escTg(basePayload.title)}\n${escTg(basePayload.message)}\n\n<a href="${link}">Відкрити діалог</a>`;
										ccNotify.sendTelegram(wcIdChannel, tgText).catch(() => {});
									} catch (e) {
										console.error("wc telegram:", e.message);
									}
								}
							}
						} catch (e) {
		...
		socket.on("client:lead", async ({ formId, values, trigger } = {}) => {
			const d = socket.data || {};
			console.log("[LEAD] прийшов:", JSON.stringify({ formId, values, trigger, role: d.role, room: d.roomId }));
			if (d.role !== "client" || !d.roomId) return;
			...
			try {
				...
				// Дублюємо лід у головну CRM-таблицю ({prefix}leads) → розділ /leads/
				try {
					const now = new Date().toISOString().slice(0, 19).replace("T", " ");

					// Дані відвідувача (UTM, пристрій, сторінка) — з visitor_meta
					let vm = {};
					let firstSeen = null;
					try {
						const metaRow = await getVisitorMeta(d.siteId, d.visitorId);
						vm = (metaRow && metaRow.data) || {};
						firstSeen = metaRow && metaRow.firstSeen ? metaRow.firstSeen : null;
					} catch (e) {}

					// ── Парсер userAgent (без залежностей) ──
					function parseUA(ua) {
						ua = String(ua || "");
						let browser = "";
						let os = "";
						...
						// Браузер (порядок важливий: Edge/Opera до Chrome, Chrome до Safari)
						let m;
					...
					const uaInfo = parseUA(vm.userAgent);

					// Час сесії (сек) — від першого візиту до створення ліда
					let sessionSec = "";
					if (firstSeen) {
						try {
							const diff = Math.round((Date.now() - new Date(firstSeen).getTime()) / 1000);
							if (diff >= 0 && diff < 86400 * 30) sessionSec = String(diff);
						} catch (e) {}
					}

					// Повна сторінка, звідки заявка
					const pageFull = vm.pageUrl || pageUrl || "";

					// contact_info у форматі картки ліда
					const contactInfo = {
						name: known.name || "",
						phone: known.phone || "",
						email: known.email || "",
					...
					const utm = {
						...
						term: u.term || "",
						content: u.content || "",
						referrer: vm.referrer || "",
					};
					const hasUtm = Object.values(utm).some((v) => v);

					// Fingerprint (ip/country/city/browser/os/device/page/session_sec + сирі)
					const fingerprint = {
						ip: vm.ip || "",
						country: "", // GeoIP — потребує бази, поки порожньо
						city: "",
						browser: uaInfo.browser || "",
						os: uaInfo.os || vm.platform || "",
						device: uaInfo.device || (vm.touch === "yes" ? "mobile" : "desktop"),
						page: pageFull,
					...
					const hasFp = Object.values(fingerprint).some((v) => v);

					// ── Товар (картка) → ціна в value + products ──
					let leadValue = "0.0000";
					let productsJson = null;
					try {
						const prod = visitorProduct.get(d.roomId) || null;
						let chosen = prod;
						// якщо поточного нема — беремо останній з історії
						if (!chosen) {
							const hist = await getProductHistory(d.siteId, d.visitorId, null, 1).catch(() => []);
							if (hist.length) chosen = hist[0].product;
						}
						if (chosen) {
							productsJson = JSON.stringify([chosen]);
							// ціна: лишаємо тільки число (прибираємо валюту/пробіли/коми)
							const priceNum = parseFloat(
								String(chosen.price || "")
									.replace(/[^\d.,]/g, "")
									.replace(",", ".")
							);
							if (!isNaN(priceNum) && priceNum > 0) leadValue = priceNum.toFixed(4);
						}
					} catch (e) {}

					// Логічна назва
					const triggerLabel = tr === "offline" ? "поза робочим часом" : "оператор не відповів";
					const whoName = known.name || known.phone || known.email || "відвідувач";
					const leadTitle = `Веб-чат: ${whoName} (${triggerLabel})`;

					// Пайплайн 1, статус 1; стадія — перша активна в пайплайні
					const LEAD_PIPELINE = 1;
					const LEAD_STATUS = 1;
					let leadStageId = null;
					try {
						const [stg] = await pool.query(`SELECT id FROM ${prefix}leads_pipeline_stages WHERE id_pipeline = ? AND is_active = 1 ORDER BY sort ASC LIMIT 1`, [LEAD_PIPELINE]);
						if (stg.length) leadStageId = stg[0].id;
					} catch (e) {}

					await pool.query(
						`INSERT INTO ${prefix}leads
                            (title, note, value, id_pipeline, id_stage, id_status, priority, lead_source, website,
                             capture_type, capture_ref, contact_info, utm, fingerprint, products, custom_fields, date_add, date_edit)
                         VALUES (?, '', ?, ?, ?, ?, 1, 'webchat', ?, 'webchat', ?, CAST(? AS JSON),
                                 ${hasUtm ? "CAST(? AS JSON)" : "NULL"},
                                 ${hasFp ? "CAST(? AS JSON)" : "NULL"},
                                 ${productsJson ? "CAST(? AS JSON)" : "NULL"},
                                 CAST(? AS JSON), ?, ?)`,
						(function () {
							const params = [leadTitle, leadValue, LEAD_PIPELINE, leadStageId, LEAD_STATUS, pageFull || d.siteId, String(d.roomId), JSON.stringify(contactInfo)];
							if (hasUtm) params.push(JSON.stringify(utm));
							if (hasFp) params.push(JSON.stringify(fingerprint));
							if (productsJson) params.push(productsJson);
							params.push(JSON.stringify(chk.values || {}));
							params.push(now, now);
							return params;
						})()
					);
				} catch (e) {
					console.error("[LEAD] CRM leads FAIL:", e.code, e.sqlMessage || e.message);
				}
				...
				// ── Повідомлення-картка з даними форми ДЛЯ МЕНЕДЖЕРА ──
				// Зберігаємо як звичайне повідомлення (from=client), щоб воно
				// лягло в стрічку і задзеркалилось у CRM через ccBridge.mirror.
				try {
					// Мапимо name-поля у людські лейбли з конфіга форми
					const labelOf = {};
					(formDef.fields || []).forEach(function (f) {
						labelOf[f.name] = (f.label && typeof f.label === "object" ? f.label[cfg.locales && cfg.locales.primary] || f.name : f.label) || f.name;
					});
					const leadLines = Object.entries(chk.values).map(function (pair) {
						return (labelOf[pair[0]] || pair[0]) + ": " + pair[1];
					});
					const trLabel = tr === "offline" ? "поза робочим часом" : tr === "timeout" ? "оператор не відповів" : "форма";
					const leadMsgText = "📋 Клієнт залишив заявку (" + trLabel + "):\n" + leadLines.join("\n");

					// Пишемо в стару схему як повідомлення клієнта, тип lead_data
					const leadMsgId = await saveMessage({
						siteId: d.siteId,
						idChat: d.roomId,
						sender: "client",
						text: leadMsgText,
					...
					// Операторам (стара сторінка)
					io.to(`operators_${d.siteId}`).emit("operator:message", { roomId: d.roomId, siteId: d.siteId, message: { id: leadMsgId, from: "client", text: leadMsgText, type: "lead_data" } });

					// Дзеркалимо в CRM-схему → зʼявиться на новій сторінці діалогу
					ccBridge.mirror(d.siteId, d.roomId, "in", { id: leadMsgId, text: leadMsgText, date_add: new Date() }).catch(function (e) {
						console.error("[WC lead mirror]", e.message);
					});
				} catch (e) {
					console.error("lead data message:", e.message);
				}

				// Підтвердження КЛІЄНТУ (як було)
...
// Менеджер відредагував повідомлення в CRM → оновити стару веб-чат схему + сповістити клієнта.
// oldMsgId — id у web_chat_messages (з source_id "wc_<id>").
module.exports.editMessageFromCrm = async function (siteId, roomId, oldMsgId, text) {
	if (!io) return { ok: false };
	try {
		await editMessage(siteId, roomId, oldMsgId, text); // оновлює стару схему
		io.to(roomId).emit("client:message_edited", { id: oldMsgId, text: text }); // віджет покаже "(змінено)"
		io.to(`operators_${siteId}`).emit("operator:message_edited", { roomId, siteId, id: oldMsgId, text: text });
		return { ok: true };
	} catch (e) {
		console.error("editMessageFromCrm:", e.message);
		return { ok: false };
	}
};

// Менеджер друкує в CRM-діалозі → шлемо індикатор клієнту веб-чату
module.exports.typingFromCrm = async function (siteId, roomId) {
	if (!io) return;
	try {
		const cfg = await getSiteConfig(siteId);
		if (!cfg.operatorTyping) return; // вимкнено в налаштуваннях каналу
	} catch (e) {
		// якщо конфіг не прочитався — за замовчуванням шлемо
	}
	io.to(roomId).emit("typing", { role: "operator" });
};

// Список форм каналу для CRM-діалогу (з target для групування).
module.exports.getFormsForCrm = async function (siteId) {
	try {
		const cfg = await getWidgetConfig(siteId);
		if (!cfg || !cfg.forms) return [];
		const primary = cfg.locales && cfg.locales.primary;
		return Object.entries(cfg.forms).map(([id, f]) => ({
			id,
			target: f.target || "quick",
			label: pickText(f.intro, primary, primary) || id,
			fieldsCount: Array.isArray(f.fields) ? f.fields.length : 0,
		}));
	} catch (e) {
		console.error("getFormsForCrm:", e.message);
		return [];
	}
};

// Надіслати форму клієнту з CRM-діалогу (та сама логіка, що operator:send_form).
module.exports.sendFormFromCrm = async function (siteId, roomId, formId, managerId) {
	if (!io) return { ok: false, error: "Socket веб-чату не ініціалізовано" };
	try {
		const cfg = await getWidgetConfig(siteId);
		const fid = String(formId || "");
		const formDef = cfg && cfg.forms && cfg.forms[fid];
		if (!formDef) return { ok: false, error: "Форму не знайдено" };

		// мова клієнта з активного сокета
		let lang = "";
		for (const [, s] of io.sockets) {
			const sd = s.data || {};
			if (sd.role === "client" && sd.roomId === roomId) {
		...
		const primary = cfg.locales && cfg.locales.primary;
		const title = pickText(formDef.intro, pickLocale(cfg, lang), primary) || "Будь ласка, заповніть форму";

		const id = await saveMessage({ siteId, idChat: roomId, sender: "operator", managerId: managerId || 0, text: title, type: "lead_prompt", meta: { trigger: "manual", formId: fid } });
		io.to(roomId).emit("client:message", { id, text: title, leadPrompt: true, trigger: "manual", formId: fid, timestamp: new Date().toISOString() });
		io.to(`operators_${siteId}`).emit("operator:message", { roomId, siteId, message: { id, from: "operator", text: title, type: "lead_prompt" } });

		// Не засмічуємо стрічку CRM повідомленням "надіслано форму" —
		// цінна лише відповідь клієнта (прийде як заявка lead_data).
		// Менеджер бачить факт надсилання через notyf на фронті.

		return { ok: true, id };
	} catch (e) {
		console.error("sendFormFromCrm:", e.message);
		return { ok: false, error: e.message };
	}
};
...
module.exports.getVisitorInfo = async function (siteId, roomId) {
	// visitorId = частина roomId після "<siteId>_". roomId = "<siteId>_v_<hex>".
	const idx = String(roomId).indexOf("_v_");
	const visitorId = idx > -1 ? String(roomId).slice(idx + 1) : String(roomId).slice(String(siteId).length + 1);
	try {
